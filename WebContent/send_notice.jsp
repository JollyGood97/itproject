...
<%	
	...
	if(id == 1){ //Sending notice to a particular teacher
		...
		if(rs.next()) {
			int i = st.executeUpdate("insert into `notice` (`descr`, `sdate`, `sid`) values ('"+desc+"','"+sdate+"','"+sid+"')");
		...
		ResultSet rs = st.executeQuery("select * from student where `id` = '" + stid + "'");
	
		if(rs.next()) {
			int i = st.executeUpdate("insert into `notice` (`descr`, `sdate`, `sid`) values ('"+desc+"','"+sdate+"','"+sid+"')");
			int j = st.executeUpdate("insert into `send_to` (`sid`) values ('"+stid+"')");
		...
		if(rs.next()) {
			cid = rs.getString(1);
			int i = st.executeUpdate("insert into `notice` (`descr`, `sdate`, `sid`) values ('"+desc+"','"+sdate+"','"+uid+"')");
